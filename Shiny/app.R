...
ui <- fluidPage( 
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("LDSD", label = "rate of VZ prolif during LD --> SD", value = 1.5, min = 0, max = 5, step = 0.01),
      
      numericInput("SDLD", label = "rate of VZ prolif during SD --> LD", value = 1.5, min = 0, max = 5, step = 0.01)
...
server <- function(input, output) {
  ...
  for (n in 1:(length(t))){ 
    ...
    proliferationVZ[n] <- reactive({
      
      input$LDSD  #rate of VZ prolif during LD --> SD
    })
